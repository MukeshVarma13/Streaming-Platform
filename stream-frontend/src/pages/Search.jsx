...
const Search = () => {
  ...
  return (
    <div className="w-full h-fit md:px-[8vw] px-[5vw] flex flex-col gap-2">
      ...
      {channelUser?.content?.length > 0 && (
        <div className="w-full mt-2">
          ...
          <div className="flex flex-col">
            {channelUser?.content.map((channel, index) => (
      ...
      {titleVideo?.content.length > 0 && (
        <div className="w-full mt-2">
          <p className="my-1 text-2xl text-grade">Title</p>
          <SearchVideosContainer videos={titleVideo.content} />
        </div>
      )}
      {descVideos?.content.length > 0 && (
        <div className="w-full">
          <p className="my-1 text-2xl text-grade">Description</p>
          <SearchVideosContainer videos={descVideos.content} />
        </div>
      )}
    </div>
  );
