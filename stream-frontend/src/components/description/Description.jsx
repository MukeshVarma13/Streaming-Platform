...
const Description = ({ streamData }) => {
  const [followers, setFollowers] = useState(
    streamData?.searchUser?.followers?.length
  );

  return (
    <div className="">
      <h1 className="text-xl mb-1.5">
        About {streamData?.streamSearch?.streamerName}
      </h1>
      <div className="w-full bg-theme rounded-sm flex justify-between px-4 flex-wrap">
        <div className="flex flex-col gap-3 py-3">
          ...
          <p className="first-letter:capitalize">{streamData?.streamSearch.description}</p>
        </div>
